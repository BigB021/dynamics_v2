...
const GlobalMusicPlayer = ({ currentTrack, onClose, theme = 'dark' }) => {
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setIsLiked(!isLiked)} className={`p-3 rounded-full transition-all duration-200 ${isLiked ? 'text-cyan-400 bg-cyan-500/20' : 'text-[color:var(--text-muted)] hover:text-blue-400 hover:bg-white/10'}`}>
                <Heart className={`w-5 h-5 ${isLiked ? 'fill-current' : ''}`} />
              </button>

              <button onClick={() => setIsShuffled(!isShuffled)} className={`p-3 rounded-full transition-all duration-200 ${isShuffled ? 'text-cyan-400 bg-cyan-500/20' : 'text-[color:var(--text-muted)] hover:text-blue-400 hover:bg-white/10'}`}>
              ...
              <button
                onClick={togglePlay}
                className="p-4 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105"
              >
              ...
              <button
                onClick={cycleRepeat}
                className={`relative p-3 rounded-full transition-all duration-200 ${repeatMode > 0 ? 'text-cyan-400 bg-cyan-500/20' : 'text-[color:var(--text-muted)] hover:text-blue-400 hover:bg-white/10'}`}
              >
            ...
            <div className="flex items-center gap-4 min-w-0">
              <div className="flex items-center gap-2">
                <button onClick={toggleMute} className="p-2 rounded-full text-[color:var(--text-muted)] hover:text-blue-400 hover:bg-white/10">
              ...
              <button onClick={onClose} className="p-2 rounded-full text-[color:var(--text-muted)] hover:text-white hover:bg-red-500/20">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
